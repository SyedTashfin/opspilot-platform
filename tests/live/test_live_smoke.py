...
Requires a provider key in the environment or in ``.env`` (for example ``DEEPSEEK_API_KEY``). Without
one it skips, so no ordinary test run and no CI job can ever spend money.
"""
...
async def test_one_live_call_reports_usage_and_cost() -> None:
    ...
    response = await gateway.complete(
        ModelRequest(
            ...
            messages=(
                ...
                Message.user("Classify this alert as database, network or deployment: 'connection "
                "pool exhausted'."),
            ),
    ...
    print(
        "\n[LIVE] provider={provider} model={model} tokens={tokens} cost_eur={cost} "
        "cost_known={known} latency_ms={latency} attempts={attempts}".format(
            provider=response.provider,
            model=response.model,
            tokens=response.usage.total_tokens,
            cost=response.cost_eur,
            known=response.cost_known,
